...
async def speak(request: SpeakRequest):
    ...
    settings = PERSONA_VOICES.get(
        request.voice.lower(),
        {"voice": request.voice, "pitch": request.pitch, "rate": request.rate},
    )
    ...
    try:
        communicate = edge_tts.Communicate(
            request.text, settings["voice"], pitch=settings["pitch"], rate=settings["rate"]
        )
