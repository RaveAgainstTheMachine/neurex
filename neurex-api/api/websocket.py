...
async def _persist_message(
    conversation_id: str, role: str, content: str, graph_id: str | None = None, workspace_path: str | None = None
):
...
async def websocket_endpoint(
    ...
    workspace_path = websocket.query_params.get("workspace_path")
    if workspace_path:
        # DO NOT mutate os.environ globally because it causes race conditions between users/tabs
        # Use contextvars to scope the path to this specific WebSocket async task
        from core.mcp.tools.filesystem import workspace_path_var
        workspace_path_var.set(workspace_path)
        log.info("ws.workspace_updated", path=workspace_path)
    ...
    async with AsyncSession(engine, expire_on_commit=False) as session:
        ...
        def get_output_handler(sid: str):
            if sid not in handlers:
                handlers[sid] = lambda data: asyncio.create_task(on_terminal_output(sid, data))
            return handlers[sid]

        # Initial default session — start in the active workspace if resolved
        import os as _os

        from api.routes.files import get_workspace
        _resolved_ws = get_workspace()
        _workspace_cwd = (
            str(_resolved_ws)
            if _resolved_ws
            else (
                _os.getenv("WORKSPACE_PATH")
                or str(Path(_os.path.abspath(__file__)).parent.parent.parent)  # neurex-api/../..
                or _os.getcwd()
            )
        )
        default_pty = pty_manager.get_or_create_session(
            conversation_id, get_output_handler(conversation_id), cwd=_workspace_cwd
        )
        ...
        try:
            while True:
                ...
                if msg_type == "terminal_sync":
                    ...
                    s = pty_manager.get_or_create_session(
                        pty_sid, get_output_handler(requested_sid), cwd=cwd, rows=rows, cols=cols
                    )
                    if requested_sid not in attached_sessions:
                        attached_sessions.add(requested_sid)
                    if s.history:
                ...
                if msg_type == "message":
                    ...
                    await _persist_message(conversation_id, "user", content, workspace_path=workspace_path)
                    ...
                    try:
                        async for event in orch.run(
                            ...
                            elif event.get("event") == "replace_tokens":
                                assistant_tokens = [event["data"]]
                        ...
                        if last_graph_id:
                            try:
                                ...
                                from core.mcp.tools.filesystem import get_workspace_root, write_file
                                
                                ws_root = get_workspace_root()
                                safe_id = re.sub(r"[^a-zA-Z0-9_-]", "", str(last_graph_id))
                                
                                rel_plan_path = ".neurex/plans/plan.md"
                                plan_path = ws_root / rel_plan_path

                                plan_content = "# ⬡ Execution Plan\n\n"
                                plan_content += "The following tasks have been scheduled to fulfill your request:\n\n"
                                for i, node in enumerate(graph, 1):
                                    desc = node.description if node.description else "No description provided."
                                    checkbox = "[x]" if node.status.value == "DONE" else "[ ]"
                                    plan_content += f"### `{checkbox}` {i}. **{node.title}** (`{node.agent_type}`)\n"
                                    plan_content += f"*Status:* `{node.status.value}`\n\n"
                                    plan_content += f"{desc}\n\n---\n\n"

                                await write_file(rel_plan_path, plan_content, autonomy_level="full")

                                chat_reply = f"### ⬡ Execution Plan\n\nI have generated an execution plan to fulfill your request. [View Plan Details](file://{plan_path.absolute()})"

                                await _persist_message(
                                    conversation_id, "assistant", chat_reply, last_graph_id, workspace_path
                                )
                                await websocket.send_json(
                                    {
                                        "event": "chat_reply",
                ...
                if msg_type == "approve_plan":
                    ...
                    # Transition planner node to DONE and other awaiting tasks to PENDING
                    try:
                        from fastapi.encoders import jsonable_encoder
                        from sqlmodel import select

                        from core.task_graph import TaskNode, TaskStatus
                        stmt = select(TaskNode).where(TaskNode.graph_id == graph_id)
                        res = await session.exec(stmt)
                        db_nodes = res.all()
                        for node in db_nodes:
                            changed = False
                            if node.agent_type == "planner" and node.status != TaskStatus.DONE:
                                node.status = TaskStatus.DONE
                                changed = True
                            elif node.status == TaskStatus.AWAITING_APPROVAL:
                        ...
                        log.error("ws.approve_plan_db_error", error=str(db_err))
