...
DEFAULT_SETTINGS = {
    ...
    "enable_model_recommendations": True,
    ...
    "background_read_autonomy_level": "full",
    "background_write_autonomy_level": "limited",
    ...
    # LLM Advanced
    "llm_temperature": 0.7,
    "llm_context_length": 1000000,
    "llm_hardware_context": 0,
    # Workspace
    ...
    # Repositories
    "skill_repositories": ["https://github.com/neurex-swarm/claude-skills"],
    "mcp_server_repositories": ["https://github.com/neurex-swarm/awesome-mcp-servers"],
}
...
class SettingsManager:
    ...
    def get_all(self) -> dict[str, Any]:
        ...
        for k, v in self.global_settings.items():
            if k == "model_routes" and isinstance(v, dict) and "model_routes" in merged and isinstance(merged["model_routes"], dict):
                merged["model_routes"] = merged["model_routes"].copy()
                merged["model_routes"].update(v)
            else:
                merged[k] = v
                
        # Merge workspace settings
        for k, v in self.workspace_settings.items():
            if k == "model_routes" and isinstance(v, dict) and "model_routes" in merged and isinstance(merged["model_routes"], dict):
                merged["model_routes"] = merged["model_routes"].copy()
                merged["model_routes"].update(v)
            else:
    ...
    def get(self, key: str) -> Any:
        ...
        if key == "model_routes":
            # Recursively merge model_routes specifically
            routes = DEFAULT_SETTINGS.get("model_routes", {}).copy()
            if "model_routes" in self.global_settings and isinstance(self.global_settings["model_routes"], dict):
                routes.update(self.global_settings["model_routes"])
            if "model_routes" in self.workspace_settings and isinstance(self.workspace_settings["model_routes"], dict):
                routes.update(self.workspace_settings["model_routes"])
            return routes
