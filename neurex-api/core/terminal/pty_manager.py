...
class PTYManager:
    ...
    def get_or_create_session(
        ...
        if session_id not in self.sessions:
            ...
            session = PTYSession(session_id, cwd=cwd, rows=rows, cols=cols)
            self.sessions[session_id] = session
            session.start()
        else:
            existing = self.sessions[session_id]
            # Session exists — if cwd given, navigate it there
            if cwd and os.path.isdir(cwd):
                if os.path.realpath(existing.workspace) != os.path.realpath(cwd):
                    log.info("pty.cd_existing", session_id=session_id, cwd=cwd)
                    existing.workspace = os.path.realpath(cwd)
                    existing.write(f"cd {cwd}\r")

            # Apply size updates if they changed
            if rows != existing.rows or cols != existing.cols:
                log.info("pty.resize_on_sync", session_id=session_id, rows=rows, cols=cols)
...
class PTYSession:
    def __init__(self, session_id: str, cwd: str | None = None, rows: int = 24, cols: int = 80) -> None:
        ...
        if requested_workspace and os.path.exists(requested_workspace):
            self.workspace = os.path.realpath(requested_workspace)
        else:
            self.workspace = os.path.realpath(os.getenv("WORKSPACE_PATH", os.getcwd()))
    ...
    def start(self) -> None:
        try:
            ...
            self.proc = PtyProcessUnicode.spawn([shell], cwd=self.workspace, env=env, dimensions=(self.rows, self.cols))
            log.info("pty.started", session=self.session_id, pid=self.proc.pid, cwd=self.workspace)
    ...
    async def _read_loop(self) -> None:
        """
        Reads from PTY using a background thread and asyncio.Queue to prevent blocking
        the async loop, ensuring zero-latency input and aggregated output.
        """
        import threading
        ...
        def producer():
            try:
                while self.proc and self.proc.isalive():
                    # read() is blocking but runs in this dedicated background thread
                ...
                log.error("pty.producer_error", session=self.session_id, error=str(pe))
            finally:
        ...
        try:
            while True:
                ...
                buffer = [data]
                # Drain the queue to batch/aggregate subsequent chunks immediately
