...
class SecuritySentinel:
    ...
    async def audit_workspace(self) -> dict[str, Any]:
        ...
        for root, _, files in os.walk(self.workspace):
            for file in files:
                if file.endswith(".py"):
                    ...
                    if any(part.startswith(".") or part == "venv" or part == "node_modules" for part in rel_path.split(os.sep)):
                        continue
        ...
        return {
            ...
            "issues": all_issues,
            "summary": {
                "files_scanned": sum(1 for _, _, files in os.walk(self.workspace) for f in files if f.endswith(".py")),
