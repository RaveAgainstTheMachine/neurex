...
class InfrastructureManager:
    ...
    async def start_engine(self, name: str):
        ...
        if name == "ollama":
            ...
            ctx_size = vram_pool.get_effective_context_tokens()

            # Example vLLM start: python -m vllm.entrypoints.openai.api_server --port 8002 --max-model-len <ctx_size>
            cmd = [
                python_exe,
                "-m",
            ...
            log.info("infra.engine_started", engine=name, port=port, max_model_len=ctx_size)
            return True
            ...
            # 2. Start llama-cpp-python server with the detected local+mesh context capacity
            cmd = [
                python_exe,
                "-m",
                "llama_cpp.server",
            ...
            log.info("infra.engine_started", engine=name, port=port, n_ctx=ctx_size, mesh_peers=len(rpc_hosts))
            return True
    ...
    def get_system_vram(self) -> float:
        """
        Estimate total available VRAM in GB across all local GPUs from supported vendors:
        NVIDIA (nvidia-smi), AMD (sysfs card total and rocm-smi), Intel (xpu-smi), macOS Apple Silicon.
        """
        import glob
        import re
        import subprocess
        ...
        try:
            ...
            for line in result.stdout.strip().split("\n"):
                if line.strip():
                    total_vram_bytes += float(line.strip()) * 1024 * 1024  # MiB to bytes
        except Exception:
            pass

        # 2. Probe AMD via sysfs (standard Linux driver)
        try:
            for path in glob.glob("/sys/class/drm/card*/device/mem_info_vram_total"):
                with open(path) as f:
                    val = f.read().strip()
                    if val.isdigit():
                        total_vram_bytes += int(val)
        except Exception:
            pass

        # If sysfs wasn't found or returned nothing for AMD, try rocm-smi
            ...
            try:
                ...
                if result.returncode == 0:
                    for line in result.stdout.split("\n"):
                        match = re.search(r"VRAM:\s*\d+\s*\(Used\)\s*/\s*(\d+)\s*\(Total\)", line)
                        if match:
                            total_vram_bytes += int(match.group(1))
            except Exception:
                pass

        # 3. Probe Intel discrete GPUs via xpu-smi
        try:
            result = subprocess.run(
                ["xpu-smi", "discovery"],
                capture_output=True,
            ...
            if result.returncode == 0:
                for line in result.stdout.split("\n"):
                    match = re.search(r"Memory Size:\s*(\d+(?:\.\d+)?)\s*([GKM]B)", line, re.IGNORECASE)
                    if match:
                        val = float(match.group(1))
                        unit = match.group(2).upper()
                        if unit == "GB":
                            total_vram_bytes += int(val * 1024 * 1024 * 1024)
                        elif unit == "MB":
                            total_vram_bytes += int(val * 1024 * 1024)
                        elif unit == "KB":
                            total_vram_bytes += int(val * 1024)
        except Exception:
            pass

        # 4. Probe Apple Silicon macOS system information
        if sys.platform == "darwin":
            try:
                ...
                if result.returncode == 0:
                    for line in result.stdout.split("\n"):
                        match = re.search(r"VRAM(?:\s*\(Dynamic,\s*Max\))?:\s*(\d+)\s*(GB|MB)", line, re.IGNORECASE)
                        if match:
                            val = int(match.group(1))
                            unit = match.group(2).upper()
                            if unit == "GB":
                                total_vram_bytes += val * 1024 * 1024 * 1024
                            else:
                                total_vram_bytes += val * 1024 * 1024
            except Exception:
                pass

            # Fallback for unified memory on Apple Silicon
            if total_vram_bytes == 0:
                try:
                    result = subprocess.run(
                        ["sysctl", "-n", "hw.memsize"],
                    ...
                    if result.returncode == 0 and result.stdout.strip().isdigit():
                        sys_mem = int(result.stdout.strip())
                        ...
                        try:
                            ...
                            if "Apple" in res_brand.stdout:
                                is_apple_silicon = True
                        except Exception:
                            pass
                        
                        if is_apple_silicon:
                            total_vram_bytes = int(sys_mem * 0.75)
                except Exception:
                    pass

        if total_vram_bytes > 0:
            total_gb = round(total_vram_bytes / (1024 * 1024 * 1024), 1)
            log.info("infra.vram_detected", total_vram_gb=total_gb)
            return total_gb

        log.warning("infra.vram_detection_failed", error="No supported GPUs detected or zero VRAM reported")
        return 0.0
