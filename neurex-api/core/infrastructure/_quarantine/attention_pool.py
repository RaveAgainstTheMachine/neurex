...
class AttentionCoordinator:
    ...
    async def distribute_attention(self, session_id: str, prompt: str, total_heads: int = 32):
        ...
        for i, peer in enumerate(peers):
            ...
            shard = AttentionShard(
                ...
                head_range=(
                    0,
                    total_heads,
                ),  # For sharded context, we might compute all heads on a shard
                context_slice=(
                    c_shard_index * self.sharder.shard_size,
                    (c_shard_index + 1) * self.sharder.shard_size,
                ),
            )
            shards.append(shard)
        ...
        log.info(
            "attention.pool_initialized",
            session=session_id,
            shards=len(shards),
            context_shards=len(context_shards),
        )

        # Parallel dispatch to peer nodes
        tasks = [
            self._dispatch_shard(session_id, shard, context_shards[i % len(context_shards)])
    ...
    def _aggregate_heads(self, results: list[Any], session_id: str) -> dict[str, Any]:
        ...
        for res in valid_results:
            asyncio.create_task(
                kv_sync.propagate_hidden_state(session_id, res.get("state"), res.get("node"))
            )
