...
class GradientHub:
    ...
    async def submit_delta(self, adapter_id: str, delta: Any, node_id: str):
        ...
        async with self.aggregation_lock:
            ...
            self.pending_deltas[adapter_id].append(
                {"node": node_id, "delta": delta, "timestamp": asyncio.get_event_loop().time()}
            )
    ...
    async def _aggregate_deltas(self, adapter_id: str):
        ...
        async with self.aggregation_lock:
            ...
            log.info(
                "gradient_hub.aggregating_weights", adapter=adapter_id, delta_count=len(deltas)
            )
            ...
            adapter = next(
                (a for a in evolution_coordinator.adapters.values() if a.id == adapter_id), None
            )
            if adapter:
                local_path = await weight_sync.finalize_local_mutation(
                    adapter_id, adapter.version, b"merged_weights"
                )
