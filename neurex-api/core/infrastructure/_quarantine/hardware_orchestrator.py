...
class HardwareOrchestrator:
    ...
    async def provision_inference_burst(
        self, session_id: str, model_id: str, estimated_vram_gb: float
    ):
        """
        ...
        """
        log.info(
            "hardware_orchestrator.provisioning_burst",
            session=session_id,
            model=model_id,
        ...
        if not plan:
            log.warning(
                "hardware_orchestrator.allocation_failed_trying_requant", session=session_id
            )
        ...
        if plan:
            ...
            log.info(
                "hardware_orchestrator.burst_provisioned",
                session=session_id,
                nodes=len(plan),
