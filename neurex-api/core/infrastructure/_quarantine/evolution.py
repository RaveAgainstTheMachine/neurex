...
class EvolutionCoordinator:
        ...
        async with self.evolution_lock:
            ...
            if not adapter:
                ...
                adapter = NeuralAdapter(
                    id=f"adapter-{domain}", base_model="qwen-2.5-coder", task_domain=domain
                )
    ...
    async def _trigger_evolution(self, domain: str):
        ...
        await arch_mutator.analyze_complexity_and_mutate(
            domain, {"avg_tool_rounds": 6, "repair_trigger_rate": 0.25}
        )
