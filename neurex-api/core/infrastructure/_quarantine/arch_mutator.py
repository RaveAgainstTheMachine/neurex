...
class ArchitectureMutator:
    ...
    async def analyze_complexity_and_mutate(
        self, domain: str, performance_telemetry: dict[str, Any]
    ):
        """
        ...
        async with self.mutation_lock:
            ...
            if avg_rounds > 5 and "k_proj" not in spec.target_modules:
                log.info(
                    "arch_mutator.expanding_modules",
                    domain=domain,
                    modules=spec.target_modules + ["k_proj", "o_proj"],
                )
    ...
    async def _trigger_adapter_rebuild(self, domain: str, spec: AdapterSpec):
        ...
        log.info(
            "arch_mutator.rebuilding_adapter",
            domain=domain,
            rank=spec.rank,
            modules=len(spec.target_modules),
        )
