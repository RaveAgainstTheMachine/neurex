...
class VirtualVRAMPool:
    ...
    def allocate_vram(self, required_gb: float) -> list[dict[str, Any]] | None:
        ...
        if required_gb > self.total_capacity_gb:
            log.warning(
                "vram_pool.insufficient_capacity",
                requested=required_gb,
                available=self.total_capacity_gb,
            )
            return None
        ...
        sorted_shards = sorted(
            self.shards.values(), key=lambda s: s.capacity_gb - s.used_gb, reverse=True
        )
        ...
        for shard in sorted_shards:
            ...
            allocation_plan.append({"node_id": shard.node_id, "allocated_gb": sharded_amount})
        ...
        log.info(
            "vram_pool.allocation_successful",
            requested_gb=required_gb,
            nodes_utilized=len(allocation_plan),
        )
    ...
    def release_vram(self, plan: list[dict[str, Any]]):
        ...
        for entry in plan:
            ...
            if node_id in self.shards:
                self.shards[node_id].used_gb = max(
                    0.0, self.shards[node_id].used_gb - entry["allocated_gb"]
                )

    def get_effective_context_tokens(self, model_size_gb: float = 10.0) -> int:
        """Convert pooled VRAM to usable token budget based on hardware capacity."""
        total_vram_gb = self.total_capacity_gb
        
        # Fallback to local system check if pool hasn't been synchronized yet
        if total_vram_gb <= 0:
            try:
                from core.infrastructure.manager import InfrastructureManager
                total_vram_gb = InfrastructureManager().get_system_vram()
            except Exception:
                total_vram_gb = 0.0

        if total_vram_gb <= 0:
            return 8192  # Fallback context budget when no VRAM is detected (CPU-only)

        available_for_kv = max(0.0, total_vram_gb - model_size_gb)
        
        # Heuristic: 300K tokens per GB of KV cache (conservative estimate)
        tokens_per_gb = 300_000
        raw_budget = int(available_for_kv * tokens_per_gb * 0.85)  # 15% safety margin
        
        return max(4096, min(raw_budget, 1_000_000))

    def get_pool_summary(self) -> dict[str, Any]:
        """Returns a dict summary of the VRAM shards and computed capacity."""
        node_breakdown = {}
        for node_id, shard in self.shards.items():
            node_breakdown[node_id] = {
                "capacity_gb": shard.capacity_gb,
                "used_gb": shard.used_gb,
                "status": shard.status,
            }
        return {
            "total_capacity_gb": self.total_capacity_gb,
            "effective_context_tokens": self.get_effective_context_tokens(),
