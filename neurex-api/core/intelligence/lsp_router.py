...
def get_session_for_file(file_path: str) -> tuple[Path, str, str]:
    ...
    if target == safe_root:
        ...
        raise PermissionError(
            f"Path traversal attempt blocked: {file_path!r} resolves outside workspace."
        )
...
async def ensure_file_opened(session: Any, abs_path: Path, uri: str, lang: str):
    ...
    notification = {
        ...
        "params": {"textDocument": {"uri": uri, "languageId": lang, "version": 1, "text": content}},
    }
...
async def lsp_go_to_definition(file_path: str, line: int, col: int) -> dict:
    ...
    try:
        ...
        for loc in locations:
            ...
            target_rel = (
                str(target_abs.relative_to(workspace))
                if target_abs.is_relative_to(workspace)
                else str(target_abs)
            )
            ...
            definitions.append(
                {"file": target_rel, "line": target_line, "column": target_col, "snippet": snippet}
            )
...
async def lsp_get_hover(file_path: str, line: int, col: int) -> dict:
    ...
    try:
        ...
        return {"found": True, "markdown": hover_text, "range": result.get("range")}
...
def lsp_get_diagnostics(file_path: str) -> dict:
    ...
    try:
        ...
        return {
            ...
            "error_count": len(
                [d for d in diagnostics if d.get("severity") == 1]
            ),  # Severity 1 is Error in LSP
        }
    except Exception as e:
