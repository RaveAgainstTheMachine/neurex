...
class PresenceManager:
        ...
        while True:
            ...
            for conv_id in list(self.active_connections.keys()):
                await self.update_presence(
                    conv_id, self.node_id, {"type": "compute_node", "capabilities": caps}
                )
    ...
    async def update_presence(self, conversation_id: str, user_id: str, data: dict[str, Any]):
        if (
            conversation_id in self.presence_state
            and user_id in self.presence_state[conversation_id]
        ):
    ...
    async def _sweep_zombies(self):
        ...
        while True:
            ...
            for conv_id, users in list(self.presence_state.items()):
                ...
                zombies = [
                    uid for uid, data in users.items() if now - data.get("last_ping", now) > 25
                ]
                if zombies:
                    for z in zombies:
