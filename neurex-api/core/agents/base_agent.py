            ...
            async with self._client.stream(
                ...
                async for line in resp.aiter_lines():
                    ...
                    if data.get("done"):
                        ...
                        start_indices = [i for i, c in enumerate(full_text) if c == '{']
                        for start_idx in start_indices:
                            for end_idx in range(len(full_text), start_idx, -1):
                                if full_text[end_idx-1] == '}':
                                    potential_json = full_text[start_idx:end_idx]
                                    try:
                                        ...
                                        if isinstance(parsed, dict) and "name" in parsed and "arguments" in parsed:
                                            ...
                                            yield {"type": "tool_call", "call": tc, "full_text": full_text[:start_idx].strip()}
