...
class BaseAgent(ABC):
    ...
    def __init__(
        ...
        self.assembler = VirtualContextAssembler(ctx)
        from core.mcp.tools.filesystem import get_workspace_root
        self.skeptical_memory = SkepticalMemory(str(get_workspace_root()))
    ...
    async def build_system_prompt(self, conversation_id: str, extra: str = "") -> str:
        ...
        # Stop conversational filler and thanking
        parts.append(
            "\n\n<agent_communication_rules>\n"
            "1. Do NOT thank the user, other agents, or yourself. Do not use conversational filler or politeness (e.g., 'Thank you for the plan', 'Starting task now', 'Great job').\n"
            "2. Keep your thoughts and outputs strictly technical, objective, and direct.\n"
            "3. Focus only on the facts, execution of the tasks, and technical reasoning.\n"
            "</agent_communication_rules>"
        )
    ...
    async def stream(
        ...
        resolved_model_name = model or self.model or get_default_model()
        engine = "ollama"
        if "gguf" in resolved_model_name.lower() or "/" in resolved_model_name:
            engine = "llama.cpp"
        ...
        if not hardware_budget:
            from core.infrastructure.vram_pool import vram_pool
            hardware_budget = vram_pool.get_effective_context_tokens()

        if engine == "ollama":
            options["num_ctx"] = hardware_budget
        ...
        if self.agent_type == "planner":
            ...
            final_tools.append({
                ...
                "function": {
                    ...
                    "description": "Query the local Plugin Hub (Marketplace) to discover Skills or MCP servers that could help complete the current task.",
                    "parameters": {
                        "type": "object",
                        "properties": {
                            "query": {"type": "string", "description": "Search query describing the capability needed"},
                            "plugin_type": {"type": "string", "description": "Filter by 'skill' or 'mcp'", "enum": ["skill", "mcp"]}
                        },
                        "required": ["query"],
                    },
                },
            ...
            if settings_manager.get("enable_model_recommendations"):
                final_tools.append({
                    "type": "function",
                    "function": {
                        "name": "install_and_route_model",
                        "description": "Download a model and update settings routing to map a task role (Coding/Planning) to it.",
                        "parameters": {
                            "type": "object",
                            "properties": {
                                "model_name": {"type": "string", "description": "The exact model name to pull and route (e.g. qwen2.5-coder:32b)"},
                                "task_role": {"type": "string", "description": "The task role to update routing for (e.g. Coding, Planning)"},
                                "engine": {"type": "string", "description": "The engine to use (ollama or llama.cpp), default: ollama"}
                            },
                            "required": ["model_name", "task_role"],
                        },
                    },
                })
        if final_tools:
        ...
        task_role = role_map.get(self.agent_type, "Coding")
        
        ollama_url, resolved_model, warning = await mesh_router.resolve_model_and_node(payload["model"], task_role)
        payload["model"] = resolved_model

        if warning:
            yield {"type": "token", "text": f"\n\n> [!NOTE]\n> {warning}\n\n"}

        # Re-evaluate engine based on resolved model
        if "gguf" in resolved_model.lower() or "/" in resolved_model:
            engine = "llama.cpp"
        elif "vllm" in resolved_model.lower() or "8002" in ollama_url:
            engine = "vllm"
        ...
        if engine == "llama.cpp":
            port = settings_manager.get("llama_cpp_port") or 8080
            if "localhost:11434" in ollama_url or "127.0.0.1:11434" in ollama_url:
                target_url = ollama_url.replace("11434", str(port)) + "/v1/chat/completions"
            else:
                target_url = f"{ollama_url}/v1/chat/completions"
        elif engine == "vllm":
            port = settings_manager.get("vllm_port") or 8002
            if "localhost:11434" in ollama_url or "127.0.0.1:11434" in ollama_url:
                target_url = ollama_url.replace("11434", str(port)) + "/v1/chat/completions"
            else:
                target_url = f"{ollama_url}/v1/chat/completions"
        ...
        try:
                ...
                async for line in resp.aiter_lines():
                    ...
                    if engine in ("llama.cpp", "vllm"):
                        ...
                        try:
                            ...
                            delta = openai_data.get("choices", [{}])[0].get("delta", {})
                            data = {
                                "message": {
                                    "role": "assistant",
                    ...
                    if content:
                        ...
                        # Fallback for models not using <chat> tags: if <chat> is not in system messages or full_text,
                        # and it doesn't look like a JSON block, treat the whole response as chat text.
                        use_chat_tags = "<chat>" in full_text or any(m.get("role") == "system" and "<chat>" in m.get("content", "") for m in messages)
                        
                        if use_chat_tags:
                            chat_matches = re.findall(r"<chat>(.*?)</chat>", full_text, flags=re.DOTALL)
                            last_chat_idx = full_text.rfind("<chat>")
                            last_close_idx = full_text.rfind("</chat>")
                            
                            current_chat_text = "".join(chat_matches)
                            if last_chat_idx != -1 and last_chat_idx > last_close_idx:
                                unclosed = full_text[last_chat_idx + 6:]
                            ...
                            # If it looks like a JSON block or tool call, do not stream as chat text
                            stripped = full_text.strip()
                            if not (stripped.startswith("{") or stripped.startswith("[")):
                                current_chat_text = full_text
                            else:
                                current_chat_text = ""
            ...
            raise RuntimeError(f"Inference error: HTTP {e.response.status_code}") from e
        except httpx.ConnectError as e:
            log.error("stream.connect_error", url=target_url, error=str(e))
            raise RuntimeError(f"Cannot connect to inference server at {target_url}") from e
