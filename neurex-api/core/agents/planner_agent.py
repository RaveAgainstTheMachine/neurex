...
AGENT SELECTION:
- coder       → create, edit, refactor, delete any file (MUST use for creating/modifying/implementing code or files, including mock data or text generation)
- tester      → run tests, lint, validate, verify outputs
- researcher  → fetch docs, investigate libraries, gather context (NEVER assign code/content writing or data generation tasks to researcher)
...
- CONTENT & DATA GENERATION: If the task requires generating mock data, random words, placeholder text, or populating a file with generated content, assign this task directly to the 'coder' agent, NOT the 'researcher' agent. The researcher agent is strictly for external search and local context retrieval.
...
class PlannerAgent(BaseAgent):
    ...
    async def plan(
        ...
        system_base = await self.build_system_prompt(conversation_id)
        ...
        messages, hardware_budget = await self.assembler.assemble(
            query=user_message,
            conversation_id=conversation_id,
            agent_type=self.agent_type,
            task_history=None,  # Let assembler load from SQLite
            system_prompt=system_base
        )

        messages.append({"role": "user", "content": user_content})

        async for chunk in self.stream(messages, params=params, hardware_budget=hardware_budget):
            if chunk["type"] == "token":
                ...
                for task_node in plan:
                    if isinstance(task_node, dict) and task_node.get("agent") == "researcher":
                        title = task_node.get("title", "").lower()
                        desc = task_node.get("description", "").lower()
                        trigger_words = ["generate", "create", "write", "produce", "random words", "mock data", "placeholder"]
                        if any(word in title or word in desc for word in trigger_words):
                            task_node["agent"] = "coder"
                            log.info("planner.corrected_agent_assignment", task=task_node.get("title"), old_agent="researcher", new_agent="coder")
