...
async def run_neural_harness(query: str, model: str = "qwen2.5-coder:14b") -> str:
    ...
    # Need conversation_id or fallback to global/background
    conversation_id = "background"
    
    agent = BaseAgent(RulesParser(), ContextManager())
    agent.agent_type = model

    harness = NeuralHarness(agent)

    final_result = ""
    async for event in harness.run(query, conversation_id):
...
TOOL_DEFINITION = {
    ...
    "input_schema": {
        ...
        "properties": {
            ...
            "model": {
                "type": "string",
                "description": "The model to drive the harness (default: qwen2.5-coder:14b).",
            },
        },
        "required": ["query"],
    },
