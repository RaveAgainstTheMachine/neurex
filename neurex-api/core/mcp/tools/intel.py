...
async def synthesize_project_intel() -> str:
    ...
    if (ws / "package.json").exists():
        intel["tech_stack"].append("TypeScript/Node.js")
    if (ws / "requirements.txt").exists():
        intel["tech_stack"].append("Python")
    if (ws / "docker-compose.yml").exists():
        intel["tech_stack"].append("Docker/Microservices")
        ...
        if path.exists():
            with open(path) as f:
                ...
                if "fastapi" in content:
                    intel["architecture_patterns"].append("FastAPI/REST")
                if "react" in content:
                    intel["architecture_patterns"].append("React/Frontend")
                if "sqlite" in content:
                    intel["architecture_patterns"].append("SQLite Persistence")
                if "totp" in content or "mfa" in content:
                    intel["coding_standards"].append("MFA Security")
                if "ruff" in content:
                    intel["coding_standards"].append("Ruff Linting")
    ...
    return (
        f"Project Intelligence Synthesized and saved to {intel_path.relative_to(ws)}.\n\n"
...
async def audit_codebase_health() -> str:
    ...
    if changelog_path.exists():
        ...
        results["documentation_drift"] = (
            "Audit required: Manual verification of v0.2.1-stable vs git history recommended."
        )
    else:
    ...
    if (ws / "neurex-api" / "node_modules").exists():
        results["recommendations"].append(
            "Found node_modules in API directory — ensure this is intended (usually only for web)."
        )
...
async def check_design_compliance(file_path: str) -> str:
    ...
    if hex_colors:
        findings.append(
            f"Hardcoded hex colors found: {', '.join(set(hex_colors))}. Use CSS variables from index.css."
        )
    ...
    if path.suffix == ".css":
        if "backdrop-filter" not in content and "glass" not in content.lower():
            findings.append(
                "No glassmorphism tokens detected. Ensure the component follows the premium aesthetic."
            )
    ...
    if path.suffix in [".tsx", ".jsx"]:
        if "className" in content and "__" not in content and "--" not in content:
            ...
            findings.append(
                "BEM naming convention (__ or --) not detected in classNames. Verify styling standard."
            )
