...
async def analyze_project_structure() -> str:
    ...
    stack = []
    if (ws / "package.json").exists(): stack.append("Node.js/TypeScript")
    if (ws / "requirements.txt").exists() or (ws / "pyproject.toml").exists(): stack.append("Python")
    if (ws / "Cargo.toml").exists(): stack.append("Rust")
    if (ws / "go.mod").exists(): stack.append("Go")
    if (ws / "Dockerfile").exists(): stack.append("Docker")
