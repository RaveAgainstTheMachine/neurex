...
def _validate_content_for_placeholders(path: str, content: str) -> str | None:
    """Check written content for developer placeholders, TODOs, and agent tags."""
    import re
    placeholder_patterns = [
        (r"<[a-zA-Z\-_0-9\s]*(?:placeholder|generated|researcher|coder)[a-zA-Z\-_0-9\s]*>", "agent placeholder tag"),
        (r"//\s*TODO:\s*(?:implement|add\s+logic|insert)", "TODO comment placeholder"),
        (r"#\s*TODO:\s*(?:implement|add\s+logic|insert)", "TODO comment placeholder"),
        (r"/\*\s*TODO:\s*(?:implement|add\s+logic|insert)", "TODO comment placeholder"),
        (r"<!--\s*TODO:\s*(?:implement|add\s+logic|insert)", "TODO comment placeholder"),
    ]
    ...
    for pattern, name in placeholder_patterns:
        ...
        if match:
            ...
            return (
                f"MUTATION_REJECTED: File write blocked. Your content contains a placeholder/incomplete marker "
                f"({name}): '{matched_str}'. You must write the actual, complete implementation."
            )
    
    path_lower = path.lower()
    is_code_file = any(path_lower.endswith(ext) for ext in [".py", ".ts", ".js", ".tsx", ".jsx", ".go", ".rs", ".java", ".cpp", ".c", ".h", ".cs"])
    if is_code_file:
        lazy_code_patterns = [
            (r"\n\s*\.\.\.\s*\n", "ellipsis line placeholder"),
            (r"\/\/\s*\.\.\.", "ellipsis comment placeholder"),
            (r"#\s*\.\.\.", "ellipsis comment placeholder"),
        ]
        for pattern, name in lazy_code_patterns:
            match = re.search(pattern, content)
            if match:
                matched_str = match.group(0).strip()
                return (
                    f"MUTATION_REJECTED: File write blocked. Your code contains a lazy placeholder "
                    f"({name}): '{matched_str}'. You must write the complete file contents without placeholders."
                )
...
async def write_file(path: str, content: str, autonomy_level: str = "limited") -> str:
    ...
    validation_err = _validate_content_for_placeholders(path, content)
    if validation_err:
...
async def apply_diff(path: str, search: str, replace: str, autonomy_level: str = "limited") -> str:
    ...
    validation_err = _validate_content_for_placeholders(path, new_content)
    if validation_err:
        return validation_err
