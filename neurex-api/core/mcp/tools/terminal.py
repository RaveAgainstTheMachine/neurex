...
async def run_command(
    ...
    # Resolve workspace at call time (not import time) to pick up per-connection ContextVar
    from core.mcp.tools.filesystem import get_workspace_root
    _workspace_mount = str(get_workspace_root())
    ...
    docker_cmd = [
        ...
        "1",
        "-v",
        f"{_workspace_mount}:/workspace:{mount_mode}",
        "-w",
