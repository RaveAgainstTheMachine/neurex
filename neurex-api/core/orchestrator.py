...
class Orchestrator:
    ...
    async def _generate_execution_replay(self, session: AsyncSession, graph_id: str):
        ...
        safe_id = re.sub(r"[^a-zA-Z0-9_-]", "", graph_id)
        rel_path = f".neurex/replays/execution_replay_{safe_id}.md"
        ...
        for i, node in enumerate(graph, 1):
            agent_emoji = {
                ...
                "researcher": "🔍", "reviewer": "👀", "debater": "⚖️", "swarm": "🐝"
            }.get(node.agent_type, "🤖")
            
            content += f"### {i}. {agent_emoji} `{node.agent_type}` - {node.title}\n"
            if node.description:
