...
class Orchestrator:
    ...
    def workspace(self) -> Path:
        if hasattr(self, "_workspace_override") and self._workspace_override is not None:
            return self._workspace_override
        from core.mcp.tools.filesystem import get_workspace_root
        return get_workspace_root()

    @workspace.setter
    def workspace(self, val: Path):
        self._workspace_override = val
    ...
    async def _generate_execution_replay(self, session: AsyncSession, graph_id: str):
        import datetime

        from core.mcp.tools.filesystem import write_file
        ...
        rel_path = ".neurex/replays/execution_replay.md"

        content = "# Execution Replay\n\n"
        content += f"**Graph ID**: `{graph_id}`\n"
        content += f"**Completed At**: `{datetime.datetime.now(datetime.UTC).isoformat()}`\n\n"
        content += "## Task Timeline\n\n"

        for i, node in enumerate(graph, 1):
            content += f"### {i}. `{node.agent_type}` - {node.title}\n"
            if node.description:
                content += f"**Description**:\n> {node.description}\n\n"
            content += f"**Result**:\n```\n{node.result or 'No result recorded.'}\n```\n\n"
            content += "---\n\n"

        await write_file(rel_path, content, autonomy_level="full")
        log.info("orchestrator.replay_generated", path=rel_path)
    ...
    async def run(
        ...
        try:
            ...
            for i, step in enumerate(plan):
                ...
                # Handle model schema drift (e.g., using "task" instead of "description")
                description = step.get("description", "")
                if not description and "task" in step:
                    description = step.get("task", "")
                
                if not description:
                    description = title if title != agent_type else f"{agent_type.capitalize()} step requires execution"

                # If title is still just the agent type, make it look better in the UI
                if title == agent_type:
                    title = f"{agent_type.capitalize()} Execution"
            ...
            if is_single_step:
                ...
                # Auto-approve in full/staging autonomy mode — skip user gate
                if self.autonomy_level in ("full", "staging"):
                    log.info(
                        "orchestrator.run.auto_approve_multi_step",
                        graph_id=graph_id,
                    ...
                    send_notification(
                        title="Plan Ready",
                        body=f"Neurex has a plan for: {planner_node.title} — tap to approve",
                    )
        except Exception as e:
                ...
                async with async_session() as session:
                    while True:
                        ...
                        if not tasks:
                            ...
                            try:
                                await self._generate_execution_replay(session, graph_id)
                            except Exception as e:
                                log.error("orchestrator.replay_generation_failed", error=str(e))
                        ...
                        for node in tasks:
                            try:
                                ...
                                agent = AgentClass(self.rules, self.ctx, model=model_name, autonomy_level=self.autonomy_level)
                                ...
                                # Preflight guard: skip tester if the preceding coder was blocked or wrote nothing
                                if node.agent_type == "tester":
                                    last_coder = next(
                                        (t for t in reversed(done_tasks) if t.agent_type == "coder"),
                                        None,
                                    )
                                    if last_coder:
                                        blocked_markers = (
                                        ...
                                        if any(m in (last_coder.result or "") for m in blocked_markers):
                                            err = (
                                                f"Tester skipped: the preceding coder task failed to create any files. "
                                                f"Coder result: {(last_coder.result or '')[:300]}"
                                            )
                                            log.error("orchestrator.tester_skipped_no_output", reason=err)
                                            await update_task(session, node.id, TaskStatus.FAILED, error=err)
                                            await queue.put({
                                                "event": "task_updated",
                                ...
                                # Send an empty chat reply to create a new message block for this worker agent's stream
                                await queue.put(
                                    {
                                        "event": "chat_reply",
                                        "data": {
                                ...
                                async for chunk in agent.execute(task_payload, conversation_id):
                                    if chunk["type"] == "token":
                                        event_type = "planning_token" if node.agent_type == "planner" else "token"
                                        await queue.put(
                                            {
                                                "event": event_type,
                                                "data": chunk["text"],
                                        ...
                                        full_text_clean = chunk.get("full_text")
                                        if full_text_clean is not None:
                                            await queue.put(
                                                {
                                                    "event": "replace_tokens",
                                        ...
                                        # Under limited/staging autonomy, filesystem writes go to staging and don't need approval.
                                        # Only shell needs approval in limited/staging autonomy.
                                        is_filesystem = chunk.get("tool") == "filesystem" or tool_name in ["write_file", "delete_file", "apply_diff", "replace_file_content", "multi_replace_file_content"]
                                        is_shell = chunk.get("tool") == "shell" or tool_name in ["run_command", "execute_command"]
                                        
                                        hitl_triggered = False
                                        if self.autonomy_level == "restricted":
                                            if rule == "ask" or is_filesystem or is_shell:
                                                hitl_triggered = True
                                        elif self.autonomy_level in ("limited", "staging"):
                                            if is_shell or (rule == "ask" and not is_filesystem):
                                                hitl_triggered = True
                                        
                                        if not is_safe_write and hitl_triggered:
                                            log.info(
                                ...
                                log.error("orchestrator.task_failed", task_id=node.id, error=str(e), exc_info=True)
        ...
        async def _worker():
            try:
                async with async_session() as session:
                    ...
                    # Create a new empty message block for this agent run
                    await queue.put({
                        "event": "chat_reply",
                        "data": { "content": "", "graph_id": node.graph_id }
                    })
    ...
    async def execute_inline_edit(
        ...
        try:
            ...
            if os.getenv("NEUREX_MOCK_LLM") == "true":
                ...
                if self.autonomy_level in ("limited", "staging"):
                    from core.mcp.tools.filesystem import write_file
                    await write_file(path, mock_modified, autonomy_level="staging")
            ...
            if self.autonomy_level in ("limited", "staging"):
                from core.mcp.tools.filesystem import write_file
                await write_file(path, full_modified, autonomy_level="staging")
