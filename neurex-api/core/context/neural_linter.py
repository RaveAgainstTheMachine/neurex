...
class NeuralLinter:
    ...
    async def verify_mutation(
        ...
        # Phase 2.1: Bypass if linter is disabled (default to False to prevent false failures and LLM overhead)
        if os.getenv("LINTER_ENABLED", "false").lower() != "true":
            return True, "Linter disabled."
