...
class VirtualContextAssembler:
    ...
    def _gather_hot(self, query: str, conversation_id: str | None, task_history: list[dict] | None, budget: int) -> list[dict]:
        ...
        current_tokens = self.ctx.count_messages_tokens(hot_messages)
    ...
    def _enforce_budget(
        ...
        sys_tokens = self.ctx.count_messages_tokens([sys_msg])
        ...
        while cold and self.ctx.count_messages_tokens(cold) > cold_budget:
            cold.pop()

        # 3. Trim Warm Budget (25%)
        warm_budget = int(budgets["CONTEXT_WINDOW"] * 0.25)
        while warm and self.ctx.count_messages_tokens(warm) > warm_budget:
            msg = warm[0]
            fit_chars = warm_budget * 4
            msg["content"] = msg["content"][:fit_chars] + "\n... [truncated warm context]"
            break

        # 4. Trim Hot Budget (35%)
        hot_budget = int(budgets["CONTEXT_WINDOW"] * 0.35)
        while hot and self.ctx.count_messages_tokens(hot) > hot_budget:
            if len(hot) > 1:
                hot.pop(0)
            else:
                msg = hot[0]
                fit_chars = hot_budget * 4
        ...
        total_tokens = self.ctx.count_messages_tokens(final_messages)
