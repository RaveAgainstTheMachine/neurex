"""
core/context/compression.py
Implements Neural Context Compression to maximize the effective context window.
Uses LLM-based summarization for mature modules and semantic pruning for irrelevant chunks.
"""

from __future__ import annotations

import structlog

from core.context.manager import ContextManager

log = structlog.get_logger()


class ContextCompressor:
    def __init__(self, context_manager: ContextManager):
        self.ctx = context_manager

    async def compress_context(self, original_context: str, target_tokens: int = 4000) -> str:
        """
        Compresses a large string of code context into a concise summary.
        If the context is already within limits, returns it as-is.
        """
        # Simple heuristic: 1 token ~= 4 characters
        if len(original_context) < (target_tokens * 4):
            return original_context

        log.info("context.compression_triggered", original_size=len(original_context))

        # 1. Semantic Pruning: Remove imports and excessive whitespace
        pruned = self._prune_boilerplate(original_context)

        if len(pruned) < (target_tokens * 4):
            return pruned

        # 2. Structural summary
        compressed = self.compress_structural(pruned)
        ratio = self.estimate_compression_ratio(original_context, compressed)
        log.info("context.compression_completed", original_chars=len(original_context), compressed_chars=len(compressed), ratio=ratio)
        return compressed

    def _prune_boilerplate(self, text: str) -> str:
        """Removes common boilerplate like large import blocks and license headers."""
        lines = text.split("\n")
        filtered = [line for line in lines if not line.strip().startswith(("import ", "from "))]
        return "\n".join(filtered)

    def compress_structural(self, text: str) -> str:
        """
        Replaces function bodies with '...' but RETAINS docstrings.
        This provides intent without the token cost of implementation.
        """
        lines = text.split("\n")
        summary_lines = []
        in_docstring = False
        in_skip = False

        for line in lines:
            stripped = line.strip()
            # Docstring detection
            if '"""' in stripped or "'''" in stripped:
                in_docstring = not in_docstring
                summary_lines.append(line)
                continue

            if in_docstring:
                summary_lines.append(line)
                continue

            # Structural markers
            if stripped.startswith(("def ", "class ", "async def ")):
                summary_lines.append(line)
                in_skip = True
            elif stripped == "" or not stripped.startswith(" "):
                in_skip = False
                summary_lines.append(line)
            elif not in_skip:
                summary_lines.append(line)
            elif in_skip and not summary_lines[-1].endswith("..."):
                summary_lines.append(f"{line[: line.find(stripped)]}# ... [body compressed] ...")

        return "\n".join(summary_lines)

    def compress_to_signatures(self, code: str) -> str:
        """
        Extract only class, function, and method signatures, including their decorators
        and immediate docstrings. Removes all implementation code (Tier 2 WARM context).
        """
        lines = code.split("\n")
        output = []
        in_docstring = False
        docstring_char = None
        decorator_buffer = []

        for line in lines:
            stripped = line.strip()

            if in_docstring:
                output.append(line)
                if docstring_char in stripped:
                    quotes_count = stripped.count(docstring_char)
                    # If quotes are closed
                    if quotes_count % 2 == 1 or stripped.endswith(docstring_char):
                        in_docstring = False
                continue

            if stripped.startswith(('"""', "'''")):
                char = '"""' if stripped.startswith('"""') else "'''"
                output.append(line)
                # Check if it is a multi-line docstring start or a single-line docstring
                if stripped.count(char) % 2 == 1:
                    in_docstring = True
                    docstring_char = char
                continue

            if stripped.startswith("@"):
                decorator_buffer.append(line)
                continue

            if stripped.startswith(("def ", "class ", "async def ")):
                if decorator_buffer:
                    output.extend(decorator_buffer)
                    decorator_buffer = []
                output.append(line)
                continue

            if stripped and not stripped.startswith("@"):
                decorator_buffer = []

        return "\n".join(output)

    def estimate_compression_ratio(self, original: str, compressed: str) -> float:
        """
        Estimate token compression ratio. Returns compressed size / original size as a float.
        """
        if not original:
            return 1.0
        return round(len(compressed) / len(original), 3)


# Singleton instance placeholder
# ContextCompressor requires ContextManager which is initialized in the Orchestrator.
