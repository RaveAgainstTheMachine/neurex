...
The Neurex CLI is the unified entry point for the **Neurex Sentient Substrate**. It is designed as a lightweight, native daemon (Rust) that handles the complex Dual-Layer architecture of the IDE.
...
2. **Execution Plane (Sandboxed Layer)**: 
   The Sentient Neural Agents (`CoderAgent`, `NeuralLinter`) and PTY execution terminals run exclusively inside a secure OS-level sandbox (Docker/Podman/bwrap). Agents cannot accidentally modify the host system outside of the explicitly mounted workspace directory.

## Commands

### `neurex start`
Bootstraps the substrate.
