...
def install_master():
    ...
    # 4. Autonomy level
    console.print("\n[bold magenta]4. Agent Autonomy[/bold magenta]")
    autonomy = questionary.select(
        "Default agent autonomy level?",
        choices=[
            "restricted — Every command requires human approval",
            "limited    — Safe commands run freely; dangerous ones ask first (Recommended)",
            "full       — Fully autonomous; no approval gates",
        ],
        default="limited    — Safe commands run freely; dangerous ones ask first (Recommended)"
    ).ask()
    autonomy_level = autonomy.split("—")[0].strip()
...
• [cyan]Autonomy:[/cyan]  {autonomy_level}
