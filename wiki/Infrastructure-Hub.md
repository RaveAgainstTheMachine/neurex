...
The Neurex Infrastructure Hub serves as the **Network Operations Center (NOC)** for the system platform. It provides real-time oversight of the physical and virtual assets powering the agentic system.

## 1. The Infrastructure Dashboard
Access the dashboard via the **Activity Bar** (Infrastructure tab) or the **Platform Launcher**. The dashboard provides a unified view of the entire Mesh topology.
...
- **Smart Fallbacks**: If a specific route is missing, the system automatically falls back to the global `default_model`, ensuring zero-latency execution.
