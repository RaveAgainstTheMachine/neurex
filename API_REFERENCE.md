...
### 1.1 Proxy Transparency 

When connecting via the **Neurex CLI Daemon**, all requests are proxied. To ensure session integrity and correct origin validation, the proxy injects the following headers:
...
Semantic Memory is a ChromaDB-backed vector store. All code written by agents and humans is automatically indexed by the `MemoryWorker`.
...
Returns metadata about the current Semantic Memory state.
...
[
  {
    "id": "filesystem-platform",
    "name": "Filesystem Platform",
    "status": "connected",
    "type": "core",
    "tools": [
      {
...
### `POST /api/settings/`
Update settings. Partial update is supported. This endpoint is the source of truth for the **Dynamic Model Routing** engine.
...
```json
{
  ...
  "llm_hardware_context": 0, // Enforced context limit in tokens (0 to auto-detect hardware VRAM boundaries)
  "model_routes": {
    "Planning": "gpt-4o",
...
>
> The `llm_hardware_context` parameter limits the context window (in tokens) processed by the LLM based on pooled VRAM across all discovered local and mesh accelerators. Setting it to `0` enables automatic hardware VRAM pooling and auto-detection.
