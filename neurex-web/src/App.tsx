...
function AppContent() {
  const defaultMainLayout = useMemo(() => {
    try {
      const val = localStorage.getItem("neurex-main-layout");
  ...
  const defaultVLayout = useMemo(() => {
    try {
      const val = localStorage.getItem("neurex-v-layout");
      return val ? JSON.parse(val) : undefined;
    } catch {
      return undefined;
    }
  ...
  const defaultHLayout = useMemo(() => {
    try {
      const val = localStorage.getItem("neurex-h-layout");
      return val ? JSON.parse(val) : undefined;
    } catch {
      return undefined;
    }
  ...
  const saveLayout = (key: string, layout: any) => {
    try {
    ...
    const checkStatus = async () => {
      ...
      try {
        ...
        if (!isInitialized && !state.isInitializing) {
          ...
          // Restore last open workspace on the backend or sync from backend
          try {
            const wsInfo = await api.get<{ path: string | null }>("/api/files/workspace");
            if (wsInfo && wsInfo.path) {
              useStore.setState((s) => {
                s.workspaceFolders = [wsInfo.path!];
              });
              localStorage.setItem("neurex_workspace_folders", JSON.stringify([wsInfo.path]));
            } else if (state.workspaceFolders && state.workspaceFolders.length > 0) {
              await api.post("/api/files/workspace", { path: state.workspaceFolders[0] });
            }
          } catch (err) {
            console.warn("Failed to sync workspace on startup:", err);
          }
          ...
          {isMobile ? (
            ...
            <div className="app__main-layout">
              ...
              <div className="app__body">
                <PanelGroup orientation="horizontal" className="app__panels" defaultLayout={defaultMainLayout} onLayoutChanged={(layout) => saveLayout("neurex-main-layout", layout)}>
                  <Panel panelRef={sidebarRef} defaultSize="18%" minSize="10%" maxSize="40%" className="app__sidebar">
                    {sidebarTab === "explorer" && <FileExplorer />}
                    {sidebarTab === "history"  && <ConversationList />}
                    {sidebarTab === "infra"    && <InfraPanel onExpand={handleSidebarResize} currentSize={sidebarRef.current?.getSize()?.asPercentage || 18} />}
                    ...
                    {sidebarTab === "agent"    && <AgentPanel onExpand={handleSidebarResize} />}
                  ...
                  <Panel minSize="30%" className="app__main-content">
                    <PanelGroup orientation="vertical" className="app__v-panels" defaultLayout={defaultVLayout} onLayoutChanged={(layout) => saveLayout("neurex-v-layout", layout)}>
                      <Panel minSize="20%" className="app__editor-wrapper">
                        <PresenceBar />
                        {showSettings ? <SettingsPanel /> : (
                          <PanelGroup orientation="horizontal" defaultLayout={defaultHLayout} onLayoutChanged={(layout) => saveLayout("neurex-h-layout", layout)}>
                      ...
                      <Panel 
                        ...
                        minSize="0%" 
                        collapsible={true}
                        panelRef={(ref) => { (window as any).neurexBottomPanel = ref; }}
