...
export function useWebSocket(conversationId: string) {
  ...
  useEffect(() => {
    if (!token || !conversationId || conversationId === "undefined") return;
    ...
    const connect = () => {
      ...
      socket.onopen = () => {
        ...
        // Sync all active terminal sessions with current dimensions and paths
        const activeState = useStore.getState();
        activeState.terminalSessions.forEach((session) => {
          const term = terminalRegistry.get(session.id);
          const cwd = session.cwd || activeState.workspaceFolders[0] || undefined;
