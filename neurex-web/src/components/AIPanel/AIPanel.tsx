    ...
    <div className="ai-panel">
      ...
      {tab === "chat" && (
        <>
          ...
          {/* Approval Banner */}
          {(() => {
            const pendingPlanTasks = nodes.filter(n => n.status === "AWAITING_APPROVAL");
...
const TaskCard = React.memo(function TaskCard({
  ...
  const color = (Object.prototype.hasOwnProperty.call(STATUS_COLOR, task.status) ? STATUS_COLOR[task.status] : undefined) ?? "var(--text-muted)";
    ...
    <div className={`task-card ${isActive ? "task-card--active" : ""}`}>
      <div className="task-card__header">
        ...
        <span className="task-card__status" style={{ color }}>
          ...
          {Object.prototype.hasOwnProperty.call(STATUS_LABEL, task.status) ? STATUS_LABEL[task.status] : task.status}
        </span>
      </div>
