...
import { Loader2, Trash2, CheckCircle2, XCircle, ArrowUp, Mic, Volume2, Paperclip, Shield, Plus, Brain, Terminal, Code, FlaskConical, Search, Eye, Scale, Hexagon, Activity, Zap, Package } from "lucide-react";
...
const getAgentIcon = (type: string) => {
  ...
  if (t.includes("coder") || t.includes("architect")) return <Code size={12} />;
  if (t.includes("planner")) return <Brain size={12} />;
  if (t.includes("tester")) return <FlaskConical size={12} />;
  if (t.includes("researcher") || t.includes("explorer")) return <Search size={12} />;
  if (t.includes("reviewer") || t.includes("verifier")) return <Eye size={12} />;
  if (t.includes("debater") || t.includes("judge")) return <Scale size={12} />;
  if (t.includes("swarm")) return <Hexagon size={12} />;
  if (t.includes("commander")) return <Zap size={12} />;
  if (t.includes("dependency")) return <Package size={12} />;
...
export function AIPanel({ send, conversationId, isActive = true }: AIPanelProps) {
  ...
  const [approvingModel, setApprovingModel] = useState<string | null>(null);
  const [historySearch, setHistorySearch] = useState("");
  const [stagedDiffs, setStagedDiffs] = useState<{path: string, status: string, diff: string}[]>([]);
  ...
  const nodes = useMemo(() => {
    ...
    function traverse(node: any) {
      ordered.push(node);
      node.children.sort((a: any, b: any) => new Date(a.created_at || 0).getTime() - new Date(b.created_at || 0).getTime());
  ...
  const fetchStagedDiffs = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/files/stage/diff`);
      if (res.ok) {
        const data = await res.json();
        setStagedDiffs(data);
      }
    } catch (err) {
      console.error("Failed to fetch staged diffs", err);
    }
  };

  useEffect(() => {
    if (!isWorking) {
      // Finished working, refresh tree and check staging
  ...
  const handleApproveModelUpgrade = async (recommendedModel: string, taskRole: string, engine: string) => {
    setApprovingModel(recommendedModel);
    const toastId = toast.loading(`Pulling model ${recommendedModel}...`);
    try {
      await api.post(`/api/infra/model/pull?engine=${encodeURIComponent(engine)}&model=${encodeURIComponent(recommendedModel)}`);
      
      const currentRoutes = settings?.model_routes ? { ...settings.model_routes } : {};
      currentRoutes[taskRole] = recommendedModel;
      await api.post('/api/settings/', { settings: { model_routes: currentRoutes } });
      
      await refreshSettings();
      toast.success(`Successfully installed and routed '${recommendedModel}' for task role '${taskRole}'!`, { id: toastId });
    } catch (err: any) {
      toast.error(`Model upgrade failed: ${err.message || err}`, { id: toastId });
    } finally {
      setApprovingModel(null);
    }
  };
        ...
        <>
          <div ref={scrollRef} className="ai-messages">
            ...
            {messages.filter(m => m.content && typeof m.content === 'string' && m.content.trim() !== "").map((msg, msgIdx) => {
              const isActivity = msg.role === "assistant" && msg.graph_id && !msg.content.startsWith("### ⬡ Execution Plan") && !msg.content.startsWith("❌");
              const recMatch = typeof msg.content === 'string' && msg.content.match(
                /Model Recommendation\*\*:\s*Neurex recommends\s*`([^`]+)`\s*\(Benchmark:\s*([^)]+)\)\s*for\s*`([^`]+)`\.\s*Currently routed to\s*`([^`]+)`\.\s*(?:Proposing upgrade to\s*`([^`]+)`\s*via engine\s*`([^`]+)`\.)?/i
              );
                ...
                <ReactMarkdown 
                  ...
                  components={{
                    code({ node, inline, className, children }: any) {
                          ...
                          return (
                            <details className="ai-thinking-block">
                              <summary className="ai-thinking-header">
                                <span className="ai-thinking-header-left">
                                  <Brain size={12} /> Intermediate Thinking
                                </span>
                                <span className="ai-thinking-toggle-icon">▶</span>
                              </summary>
                      ...
                      if (inline) {
                        const t = codeContent.toLowerCase().trim();
                        const isAgent = ["planner", "coder", "tester", "researcher", "reviewer", "debater", "commander", "dependency", "swarm"].includes(t);
                        if (isAgent) {
                          return (
                            <code className="agent-code-inline">
                              {getAgentIcon(t)}
                              <span>{t}</span>
                            </code>
                          );
              ...
              return (
                <div key={msg.id} id={msg.id ? `msg-${msg.id}` : `msg-idx-${msgIdx}`} className={`message message--${msg.role}`}>
                  <div className="message__content">
                    {isActivity ? (
                      <details className="ai-activity-block">
                        <summary className="ai-activity-header">
                          <span className="ai-activity-header-left">
                            <Terminal size={12} /> Agent Execution Steps
                          </span>
                          <span className="ai-activity-toggle-icon">▶</span>
                        </summary>
                ...
                {msg.role === "assistant" && recMatch && (
                  <div className="message__inline-approval">
                    <div className="inline-approval-card">
                      <div className="inline-approval-card__title">
                        <Brain size={14} style={{ flexShrink: 0 }} /> Model Upgrade Recommended
                      </div>
                      <p style={{ margin: '8px 0', fontSize: '12px', opacity: 0.9 }}>
                        Upgrade cognitive route for <strong>{recMatch[3]}</strong> to <strong>{recMatch[1]}</strong> ({recMatch[2]} benchmark score) via <strong>{recMatch[6] || 'ollama'}</strong>?
                      </p>
                      <div className="inline-approval-card__actions">
                        <button 
                          className="btn btn--purple" 
                          disabled={approvingModel === recMatch[1]}
                          onClick={() => handleApproveModelUpgrade(recMatch[1], recMatch[3], recMatch[6] || 'ollama')}
                        >
                          {approvingModel === recMatch[1] ? "Upgrading..." : "Approve & Install"}
                        </button>
                      </div>
                    </div>
                  </div>
                    ...
                    {pendingPlanTasks.map(task => (
                      <div key={task.id} className="ai-approval-tray__item">
                        <div className="ai-approval-tray__label">
                          <span className="ai-approval-tray__agent">{getAgentIcon(task.agent_type)} {task.agent_type}</span>
          ...
          {stagedDiffs.length > 0 && (
            <div className="staging-review-panel">
              <div className="staging-review-header">
                <Shield size={14} className="text-purple-400" />
                <span>Staged Changes ({stagedDiffs.length})</span>
                <div className="staging-review-actions">
                  <button
                    className="btn btn--purple btn--sm"
                    onClick={async () => {
                      try {
                        const res = await fetch(`${API_BASE}/api/files/stage/commit`, { method: "POST" });
                        if (res.ok) {
                          toast.success("Committed staged changes to workspace");
                          setStagedDiffs([]);
                          useStore.getState().refreshFileTree();
                        }
                      } catch (err) {
                  ...
                  <button
                    ...
                    onClick={async () => {
                      try {
                        const res = await fetch(`${API_BASE}/api/files/stage/clear`, { method: "POST" });
                        if (res.ok) {
                          toast.success("Discarded staged changes");
                          setStagedDiffs([]);
                        }
                      } catch (err) {
              ...
              <div className="staging-file-list">
                {stagedDiffs.map((file) => {
                  const isExpanded = !!expandedDiffs[file.path];
                  return (
                    <div key={file.path} className="staging-file-item">
                      <div
                        className="staging-file-row"
                        onClick={() => setExpandedDiffs(prev => ({ ...prev, [file.path]: !isExpanded }))}
                      >
                        <span className="staging-file-arrow">{isExpanded ? "▼" : "▶"}</span>
                        <span className={`staging-file-status status-${file.status}`}>{file.status[0].toUpperCase()}</span>
                        <span className="staging-file-path">{file.path}</span>
                      </div>
                      {isExpanded && (
                        <pre className="staging-file-diff">
                          {file.diff ? (
                            file.diff.split("\n").map((line, i) => {
                              let className = "";
                            ...
                            <div className="diff-empty">Binary file or no text changes.</div>
                          )}
                        </pre>
                      )}
                    </div>
  ...
  return (
    <div className={`task-card ${isActive ? "task-card--active" : ""}`}>
      <div className="task-card__header">
        <span className="task-card__agent">{getAgentIcon(task.agent_type)} {task.agent_type}</span>
        ...
        return (
          <div key={idx} className="plan-step-card">
            <div className="plan-step-card__header">
              ...
              <span className="plan-step-card__agent">{getAgentIcon(agent)} {agent}</span>
