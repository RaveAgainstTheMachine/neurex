...
const FileItem = React.memo(function FileItem({ node, depth, rootPath }: { 
  ...
  const handleClick = async () => {
    if (isDir) {
      ...
      if (node.path !== undefined && toggleFolder) toggleFolder(node.path);
    } else if (node.path) {
        ...
        >
          <div className="sidebar-section__header explorer-header" onClick={() => toggleSection('workspace')}>
            <div className="sidebar-section__header-left">
              ...
              <span>{workspaceFolders.length > 1 ? "WORKSPACE" : (workspaceFolders[0]?.split(/[/\\]/).pop() || "Select or Start a project").toUpperCase()}</span>
            </div>
            {workspaceFolders.length > 0 && (
              <div className="explorer-actions">
                ...
                <button className="icon-btn" onClick={(e) => { e.stopPropagation(); useStore.getState().closeWorkspace(); }} title="Close Folder"><X size={14} /></button>
              </div>
            )}
          </div>
          {sections.workspace && (
              ...
              {fileTree.length > 0 ? (
                workspaceFolders.length === 1 ? (
                  (fileTree[0]?.children || [])
                    ...
                    .sort((a, b) => (a.type === "dir" ? -1 : 1) || a.name.localeCompare(b.name))
                    .map((node) => <FileItem key={node.path || node.name} node={node} depth={0} rootPath={workspaceFolders[0]} />)
                ) : (
