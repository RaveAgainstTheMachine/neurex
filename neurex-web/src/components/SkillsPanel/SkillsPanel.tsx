...
export function SkillsPanel() {
  ...
  const [pluginType, setPluginType] = useState<"skill" | "mcp">("skill");
  ...
  const filteredMarketplace = useMemo(() => {
    return curated.filter(item => {
      ...
      const matchesType = (item.plugin_type || "skill") === pluginType;
      return matchesSearch && matchesCategory && matchesType;
    });
  }, [curated, searchQuery, marketCategory, pluginType]);
  ...
  return (
    <div className="skills-panel">
      ...
      <div className="extensions-header" style={{ flexDirection: 'column', alignItems: 'flex-start', gap: '12px' }}>
        <div style={{ display: 'flex', gap: '8px', width: '100%' }}>
          <button 
            className={`btn btn--sm ${pluginType === 'skill' ? 'btn--purple' : 'btn--muted'}`}
            onClick={() => setPluginType('skill')}
          >
            Agent Skills
          </button>
          <button 
            className={`btn btn--sm ${pluginType === 'mcp' ? 'btn--purple' : 'btn--muted'}`}
            onClick={() => setPluginType('mcp')}
          >
            MCP Servers
          </button>
        </div>
        <div style={{ display: 'flex', width: '100%', gap: '12px', justifyContent: 'space-between' }}>
          <div className="extensions-search-container" style={{ flex: 1 }}>
            <input 
              type="text" 
              placeholder={`Search ${pluginType === 'skill' ? 'Skills' : 'MCP Servers'}...`} 
                  ...
                  <div className="extension-details">
                    <div className="extension-name-row">
                      <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                        ...
                        {item.is_official ? (
                          <span style={{ backgroundColor: 'rgba(56, 189, 248, 0.1)', color: '#38bdf8', padding: '2px 6px', borderRadius: '4px', fontSize: '10px', fontWeight: 600 }}>OFFICIAL</span>
                        ) : (
                          <span style={{ backgroundColor: 'rgba(168, 85, 247, 0.1)', color: '#a855f7', padding: '2px 6px', borderRadius: '4px', fontSize: '10px', fontWeight: 600 }}>COMMUNITY</span>
                        )}
                      </div>
                      {isInstalled(item) ? (
