...
export const createEditorSlice: StoreSlice<NeurexStore> = (set, get) => ({
  // ── Editor Actions ────────────────────────────────────────────────
    ...
    openFiles: (() => {
      try {
        const val = localStorage.getItem("neurex_open_files");
    ...
    openFile: (path, content, language, isPreview = false, root?: string) => {
      ...
      set((s) => {
        ...
        if (existingIdx !== -1) {
          s.openFiles[existingIdx].isPreview = isPreview;
          s.activeFile = path;
        } else {
          if (isPreview) {
    ...
    terminalSessions: (() => {
      try {
        const val = localStorage.getItem("neurex_terminal_sessions");
        if (!val || val === "undefined") return [{ id: "default", name: "bash" }];
        const parsed = JSON.parse(val);
        return Array.isArray(parsed) ? parsed : [{ id: "default", name: "bash" }];
      } catch {
        return [{ id: "default", name: "bash" }];
      }
    })(),
