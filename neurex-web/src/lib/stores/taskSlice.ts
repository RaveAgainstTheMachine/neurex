...
export const createTaskSlice: StoreSlice<NeurexStore> = (set, _get) => ({
  ...
  upsertTask: (task: TaskNode) => set((s) => {
    ...
    s.tasks[task.id] = {
      ...existing,
      ...task,
      status: task.status ? (task.status.toUpperCase() as any) : (existing.status || "PENDING")
    };
  }),
    ...
    try {
      ...
      set((s) => {
        ...
        allTasks.forEach((t) => {
          s.tasks[t.id] = {
            ...t,
            status: t.status ? (t.status.toUpperCase() as any) : "PENDING"
          };
        });
      });
  ...
  approveTask: async (taskId) => {
    try {
      ...
      set((s) => {
        if (s.tasks[taskId]) {
          s.tasks[taskId].status = res.status ? (res.status.toUpperCase() as any) : "PENDING";
