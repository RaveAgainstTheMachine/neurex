...
export const createFiletreeSlice: StoreSlice<NeurexStore> = (set, get) => ({
  // ── File Tree ────────────────────────────────────────────────
    ...
    workspaceFolders: (() => {
      try {
        const val = localStorage.getItem("neurex_workspace_folders");
    ...
    expandedFolders: (() => {
      try {
        const val = localStorage.getItem("neurex_expanded_folders");
        if (!val || val === "undefined") return new Set<string>();
        const parsed = JSON.parse(val);
    ...
    collapsedFolders: (() => {
      try {
        const val = localStorage.getItem("neurex_collapsed_folders");
        if (!val || val === "undefined") return new Set<string>();
        const parsed = JSON.parse(val);
        return new Set<string>(Array.isArray(parsed) ? parsed : []);
      } catch {
    ...
    setWorkspace: async (path: string) => {
      try {
        ...
        let nextChatId = typeof crypto.randomUUID === 'function' ? crypto.randomUUID() : Math.random().toString(36).substring(2);
        try {
        ...
        set((s) => { 
          ...
          // Update chat
          s.activeConversationId = nextChatId;
          s.messages = [];
          s.tasks = {};
        });
        
        localStorage.setItem("neurex_conv_id", nextChatId);
        try {
          const map = JSON.parse(localStorage.getItem("neurex_workspace_chats") || "{}");
          map[path] = nextChatId;
    ...
    fetchSubtree: async (path: string) => {
      ...
      } catch (err: any) { console.error("Failed to fetch subtree:", err); }
    },
