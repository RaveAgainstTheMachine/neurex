...
## One-stop dev environment setup (deps + git hooks)
setup: install-api install-web
	git config core.hooksPath scripts/hooks
	chmod +x scripts/hooks/pre-commit
	@echo ""
	@echo "✅ Dev environment ready. Pre-commit hook active."
...
lint:
	...
	python3 scripts/verify-smoke.py
...
eval:
	cd neurex-api && ./.venv/bin/python ../eval/run_evals.py

## Run live evaluations with real LLM inference against local Ollama (requires running API)
test-live:
	cd neurex-api && NEUREX_MOCK_LLM=false ./.venv/bin/python ../eval/run_evals.py

## Run quick end-to-end smoke tests against live model
smoke:
	cd neurex-api && NEUREX_MOCK_LLM=false ./.venv/bin/python ../eval/run_evals.py --only smoke
...
## Push docs-only changes (skips CI rebuild)
release-docs:
	git add -A
